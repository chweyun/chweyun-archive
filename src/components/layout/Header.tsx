...
export default function Header() {
    ...
    const NavItem = ({ navItem }: { navItem: { name: string; href: string; src: string; isNewWindow: boolean } }) => (
        <div key={navItem.name} className="flex items-center">
            <Link href={navItem.href} className="px-1 py-1" target={navItem.isNewWindow ? "_blank" : ""}>
                <Image src={`/icons/${navItem.src}`} width="120" height="70" alt={navItem.name} className="cursor-pointer" />
            </Link>
        </div>
    );
